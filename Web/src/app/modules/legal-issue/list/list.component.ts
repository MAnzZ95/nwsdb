...
export class LegalIssueListComponent {
  ...
  async ngOnInit() {
    ...
    this.getAllLegalIssueCount();
    this.isSelectedSort();
    this.getAllLegalIssueAsync();
    this.cdr.detectChanges();
  }

  getAllLegalIssueAsync() {
    const sub = this.landService.getLegalIssueDetails().subscribe(response => {
  ...
  getAllLegalIssueCount() {
    const sub = this.landService.getLegalCount().subscribe(response => {
      if (response) {
        ...
        this.selectAllLegalIssueWithPagination(oDataQuery);
      }
    });
    this.subscription.add(sub);
  }
  ...
  async selectAllLegalIssueWithPagination(ODataQuery: HttpParams) {
    const sub = this.landService
      .getLegalDetailsWithOData(ODataQuery)
      .subscribe(response => {
        if (response) {
          const data = Array.isArray(response) ? response : [response];
          this.dataSource.data = data;
