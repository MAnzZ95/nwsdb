...
export class WssListComponent {

  dataSource: MatTableDataSource<Wss>;
  ...
  constructor(
    ...
    this.dataSource = new MatTableDataSource<Wss>([]);
  }

  async ngOnInit() {
    this.getColumnDetails();
    this.getAllWsssCount();
    this.isSelectedSort();
  ...
  getAllLandsAsync() {
    const sub = this.wssService.getWssDetails().subscribe(response => {
  ...
  getAllWsssCount() {
    const sub = this.wssService.getWsssCount().subscribe(response => {
      if (response) {
  ...
  async selectAllLandsWithPagination(ODataQuery: HttpParams) {
    const sub = this.wssService
      .getWssDetailsWithOData(ODataQuery)
      .subscribe(response => {
        if (response) {
          const data = Array.isArray(response) ? response : [response];
          this.dataSource.data = data;
  ...
  removeCompany(id: string) {
    ...
    dialogRef.afterClosed().subscribe((result: string | undefined) => {
      if (result) {
        const sub = this.wssService.removeWss(id).subscribe({
          next: () => {
